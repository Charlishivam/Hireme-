...
class Password extends BaseController
{
	...
	public function index(){
		...
		$description = 'This helps show that this account really belongs to you';
		return view('auth/forgot_password', compact('title','description'));
	}
	...
	public function reset()
	{
		...
		$reset_link = base_url('BackendAuth/password/regenerate/'.aesencrypt($reset_data, AES_KEY, 'S'));
	...
	{	
		if ($this->request->getPost('submit')) {
			...
			$userid = filter_inp($this->request->getPost('email'));
			...
			$res = $this->userModel->updatePassword($userid, $data);
			if ($res) {
				delete_cookie('reset_cook');
				$message = 'Password updated successfully';
				return redirect()->to(base_url('/admin'))->with('success_message', $message);
			}else{
