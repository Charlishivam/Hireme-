...
class Service extends API_Controller {
    ...
    public function index(){
       $this->_apiConfig([
        ...
        $result = $this->ServiceModel->_service_records();
        if($result->num_rows() > 0){
            foreach($result->result() as $key => $val){
            $features_history = !empty($val->service_features) ? json_decode($val->service_features) : array();
            $result->result()[$key]->service_features = $features_history;
        }
      ...
      public function get_service_by_subcategoryid(){
          $this->_apiConfig([
        'methods' => ['POST'],
        'key' => ['header',$this->config->item('api_fixe_header_key')],]);
        $post = json_decode(file_get_contents('php://input'));
        if(empty($post->service_subcategory) || !isset($post->service_subcategory)){
        $this->api_return(array('status' =>false,'error' => 'Subcategory ID is empty Or missing !'),self::HTTP_OK);exit;
        }
        $subcategory_id = $post->service_subcategory;
        $result = $this->ServiceModel->_get_subcatidby_records($subcategory_id);
        if($result->num_rows() > 0){
            foreach($result->result() as $key => $val){
            $features_history = !empty($val->service_features) ? json_decode($val->service_features) : array();
            $result->result()[$key]->service_features = $features_history;
    ...
    public function create(){
        ...
        if(empty($post->service_title) || !isset($post->service_title)){
            $this->api_return(array('status' =>false,'error' => 'Service  Title is empty Or missing !'),self::HTTP_OK);exit;
        }
        if(empty($post->service_price) || !isset($post->service_price)){
            $this->api_return(array('status' =>false,'error' => 'Service price is empty Or missing !'),self::HTTP_OK);exit;
        }
        if(empty($post->service_description) || !isset($post->service_description)){
            $this->api_return(array('status' =>false,'error' => 'Service description is empty Or missing !'),self::HTTP_OK);exit;
        }
        
        if(empty($post->service_category) || !isset($post->service_category)){
            $this->api_return(array('status' =>false,'error' => 'Service Category is empty Or missing !'),self::HTTP_OK);exit;
        }
        
        if(empty($post->service_subcategory) || !isset($post->service_subcategory)){
            $this->api_return(array('status' =>false,'error' => 'Service SubCategory is empty Or missing !'),self::HTTP_OK);exit;
        }
        
        if(empty($post->service_features) || !isset($post->service_features) || !is_array($post->service_features)){
            $this->api_return(array('status' =>false,'error' => 'Service Features is empty Or missing !'),self::HTTP_OK);exit;
        }
        
        $data['customer_id']            = $post->customer_id;
        $data['service_title']          = $post->service_title;
        $data['service_price']          = $post->service_price;
        $data['service_description']    = $post->service_description;
        $data['service_category']       = $post->service_category;
        $data['service_subcategory']    = $post->service_subcategory;
        $data['service_delivery']       = $post->service_delivery;
        $service_features               = $post->service_features;
        $data['service_create_at']      = date('Y-m-d H:i:s');
       
        $amtdata = array();
            foreach($service_features as $key => $value){
                $this->db->select("*");
                $this->db->from('dk_service_features');
    	        $this->db->where('service_features_id',$value);
                $result= $this->db->get()->row();
                $service_feature = $result->service_features_name;
                $amdata['service_features_name']  = $service_feature;
                $amdata['service_features_id']    = $value;
                array_push($amtdata,array('service_features_id'=>$value,'service_features_name'=>$service_feature));
            }
        $data['service_features']  = @json_encode($amtdata);
        if(!empty($post->service_image)){
            $images = array();
            
                $path = 'uploads/service/';
                $file = $this->BaseModel->_upload_Base64_image($post->service_image,$path);
                $data['service_image']     = $path.$file;
        }
        if($this->BaseModel->_inser_query('dk_service',$data)){
            $this->api_return(array('status' =>true,'message' => 'Service successfully added !'),self::HTTP_OK);exit;
        }else{
   ...
   public function update_service(){
        ...
        $post = json_decode(file_get_contents('php://input'));
         ...
         if(empty($post->service_id) || !isset($post->service_id)){
        ...
        $data['service_features']  = @json_encode($amtdata);
       if(!empty($post->service_image) && isset($post->service_image)){
            @unlink(base_url("uploads/service/".$post->service_image));
                $path = 'uploads/service/';
                $file = $this->BaseModel->_upload_Base64_image($post->service_image,$path);
                if(!empty($post->service_image) && isset($post->service_image)){
                    $data['service_image']     = $path.$file;
        ...
        if($this->BaseModel->_update_query('dk_service',$data,array('service_id'=>$service_id))){  
           $this->api_return(array('status' =>true,'message' => 'Service successfully updated !'),self::HTTP_OK);exit;
        }else{
            $this->api_return(array('status' =>false,'error' => 'Oops! Something went wrong ! Help us improve your experience by sending an error report  !'),self::HTTP_OK);exit;
        }
    }
...
//-------------------------------------------------------------------vk--
    public function list(){
        ...
        $result = $this->ServiceModel->_get_service_records($customer_id);
        if($result->num_rows() > 0){
            foreach($result->result() as $key => $val){
            $features_history = !empty($val->service_features) ? json_decode($val->service_features) : array();
            $result->result()[$key]->service_features = $features_history;
    ...
    // create by vasit
       ...
       public function service_features_list(){
       $this->_apiConfig([
            'methods' => ['GET'],
            'key' => ['header',$this->config->item('api_fixe_header_key')],
        ]);
        $result = $this->ServiceModel->service_features_get();
        if(empty($result)){
            $this->api_return(array('status' =>true,'error' => 'Data not found !'),self::HTTP_OK);exit;
        }
       
        $this->api_return(array('status' =>true,'msg' => 'Service Data found !','data'=>$result),self::HTTP_OK);exit;
    }
    
    
    //---------------------------++++---------------------------------//
       public function search_service_by_keyword_post(){
         $this->_apiConfig([
        ...
        $limit   = 50;
        $result = $this->ServiceModel->_search_by_keywords($keyword,$limit);
         if($result->num_rows() > 0){
            foreach($result->result() as $key => $val){
            $features_history = !empty($val->service_features) ? json_decode($val->service_features) : array();
            $result->result()[$key]->service_features = $features_history;
...
//-----------------------------------------------------------
  ...
  public function service_status(){
	  ...
	  if($this->BaseModel->_update_query('dk_service',$data,array('service_id'=>$service_id))){
            $this->api_return(array('status' =>true,'msg' => 'Service status successfully updated !'),self::HTTP_OK);exit;
        }else{
            $this->api_return(array('status' =>false,'error' => 'Oops! Something went wrong ! Help us improve your experience by sending an error report  !'),self::HTTP_OK);exit;
        }
    }
